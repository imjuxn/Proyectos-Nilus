...
json_original = os.path.join(base_path, os.getenv("GOOGLE_JSON"))
json_temp = os.path.join(temp_folder, os.getenv("GOOGLE_JSON"))

if not os.path.exists(json_temp):
    shutil.copy(json_original, json_temp)

creds_json_path = os.path.join(base_path, "temp_creds", os.getenv("GOOGLE_JSON"))
...
def obtener_fecha_pedido_desde_html(driver, wait_time=10):
    try:
        fecha_element = WebDriverWait(driver, wait_time).until(
            EC.presence_of_element_located((By.XPATH, "//h6[contains(@class, 'MuiTypography-h6') and contains(text(), '/') and not(contains(text(), '#')) and not(contains(text(), 'INV'))]"))
        )
        fecha_completa = fecha_element.text.strip()
        print(f"🔍 Fecha encontrada en HTML: '{fecha_completa}'")
        if '#' in fecha_completa or 'INV' in fecha_completa.upper():
            print(f"❌ Texto encontrado no es una fecha: {fecha_completa}")
            return None
        fecha_solo = fecha_completa.split()[0].replace(',', '')
        print(f"🔍 Fecha extraída: '{fecha_solo}'")
        ...
        if len(partes_fecha) == 3:
            try:
                ...
                print(f"❌ Las partes de la fecha no son números válidos: {fecha_solo}")
                return None
            if "es-AR" in driver.current_url or "AR" in driver.current_url.upper():
                dia = partes_fecha[0].zfill(2)
                mes = partes_fecha[1].zfill(2)
                año = partes_fecha[2]
                fecha_formateada = f"{dia}/{mes}/{año}"
                print(f"🇦🇷 Formato AR: {fecha_solo} → {fecha_formateada}")
            else:
                mes = partes_fecha[0].zfill(2)
                dia = partes_fecha[1].zfill(2)
                año = partes_fecha[2]
                fecha_formateada = f"{dia}/{mes}/{año}"
                print(f"🇲🇽 Formato MX: {fecha_solo} → {fecha_formateada}")
            return fecha_formateada
        else:
            print(f"❌ Formato de fecha no reconocido: {fecha_solo}")
            return None
    except TimeoutException:
        print(f"❌ No se encontró elemento de fecha en la página después de {wait_time} segundos")
        return None
    except Exception as e:
        print(f"❌ Error inesperado al obtener fecha del pedido desde HTML")
        return None
...
def procesar_pedido(driver, pedido_id, fecha_entrega, slug_shop):
    try:
        try:
            ...
            print("❌ FALTA ANULACIÓN DE PEDIDO...")
            return "FALTA_ANULACION"
        except TimeoutException:
            pass
...
time.sleep(5)
driver.find_element(By.ID, "email").send_keys(os.getenv("EMAIL_LOGIN"))
driver.find_element(By.ID, "password").send_keys(os.getenv("PASSWORD_LOGIN"))
...
try:
    ...
    for i, fila in enumerate(valores[1:], start=2):
        ...
        if pais not in ["AR", "MX"]:
            ...
            pedidos_fallidos += 1
            continue
        print(f"\n🔄 Procesando fila {i}: País={pais}, ID={pedido_id}, Fecha nueva={fecha_entrega}, Slug={slug_shop}")
        url_pedido = f"https://backoffice.nilus.co/es-AR/orders/{pedido_id}"
        print(f"\n🔄 Procesando pedido: {pedido_id}")
        print(f"🌎 País: {pais}")
        print(f"📅 Fecha nueva programada: {fecha_entrega}")
        print(f"🏪 Slug shop: {slug_shop}")
        ...
        try:
            ...
            print(f"❌ Error obteniendo fecha del pedido {pedido_id}: {e}")
            worksheet.update_cell(i, 5, "❌ Error obteniendo fecha")
            pedidos_fallidos += 1
            continue
        try:
            ...
            if pais == "AR":
                ...
                print(f"📍 Usando reglas de Argentina")
            elif pais == "MX":
                puede_modificar = puede_modificar_pedido_mexico(fecha_pedido_actual)
                print(f"📍 Usando reglas de México")
        except Exception as e:
            print(f"❌ Error validando fecha del pedido {pedido_id}: {e}")
            worksheet.update_cell(i, 5, "❌ Error validando fecha")
            pedidos_fallidos += 1
            continue
        try:
            if puede_modificar:
                print(f"✅ Pedido {pedido_id} puede modificarse. Procediendo con procesar_pedido...")
                resultado = procesar_pedido(driver, pedido_id, fecha_entrega, slug_shop)
                if resultado == True:
                    pedidos_procesados += 1
                    print(f"✅ Fila {i} procesada correctamente")
                    worksheet.update_cell(i, 5, "✅ Hecho (post-cierre)")
                elif resultado == "FALTA_ANULACION":
                    pedidos_fallidos += 1
                    print(f"❌ Fila {i} falló - Falta anulación")
                    worksheet.update_cell(i, 5, "❌ FALTA ANULACIÓN DE PEDIDO PRIMERO")
                else:
                    pedidos_fallidos += 1
                    print(f"❌ Fila {i} falló")
                    worksheet.update_cell(i, 5, "❌ Fallo")
            else:
                print(f"⏭️ Pedido {pedido_id} NO puede modificarse según reglas de negocio. Procediendo con procesar_pedido2...")
                if procesar_pedido2(driver, pedido_id, fecha_entrega, slug_shop):
                    pedidos_procesados += 1
                    print(f"✅ Fila {i} procesada correctamente (por procesar_pedido2)")
                    worksheet.update_cell(i, 5, "✅ Hecho (pre-cierre)")
                else:
                    pedidos_fallidos += 1
                    print(f"❌ Fila {i} falló (por procesar_pedido2)")
                    worksheet.update_cell(i, 5, "❌ Fallo")
        except Exception as e:
            print(f"❌ Error procesando pedido {pedido_id}: {e}")
            worksheet.update_cell(i, 5, "❌ Error procesando")
            pedidos_fallidos += 1
            continue
    print(f"\n📊 RESUMEN FINAL:")
