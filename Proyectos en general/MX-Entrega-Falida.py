...
CHANNEL_ID_NOTIFICACIONES = os.getenv("CHANNEL_NOTIFY")
...
enviar_notificacion_slack(mensaje=f"El script de entrega fallida de MÉXICO ha comenzado 🚀")
...
def extraer_id_desde_direccion(texto_html):
    decoded_text = html.unescape(texto_html)
    for linea in decoded_text.splitlines():
        if "Dirección de entrega:" in linea:
            if "-" in linea:
                partes = linea.rsplit("-", 1)
                if len(partes) == 2:
                    posible_id = partes[1].strip()
                    posible_id = posible_id.replace("<br><br>", "").strip()
                    return posible_id
    return None
    ...
    for mensaje in mensajes:
        if "files" in mensaje:
            for archivo in mensaje["files"]:
                ...
                url = archivo["url_private_download"]
                headers = {"Authorization": f"Bearer {SLACK_TOKEN}"}
                respuesta = requests.get(url, headers=headers)

                if respuesta.ok:
                    contenido = respuesta.text
                    print(f"DEBUG: Contenido del archivo '{nombre_archivo}':\n--- INICIO CONTENIDO ---\n{contenido}\n--- FIN CONTENIDO ---")
                    id_extraido = extraer_id_desde_direccion(contenido)
                    if id_extraido:
                        ids_encontrados.append(id_extraido)
                else:
                    print(f"❌ No se pudo descargar el archivo: {respuesta.status_code}")
...
for id_ in ids:
    driver.get("https://backoffice.nilus.co/es-AR/orders?country=2&status=confirmed")
    time.sleep(2)
    exito_entrega = escribir_punto_de_entrega(driver, id_, 0)
    if not exito_entrega:
        print(f"❌ No se pudo colocar el punto de entrega para el ID: {id_}. Continuando...")
        continue
    time.sleep(3)
    fecha_hoy = datetime.now().strftime("%m/%d/%Y")
    exito_fecha = escribir_fecha_entrega(driver, fecha_hoy, 0)
    if not exito_fecha:
        print(f"❌ No se pudo escribir la fecha de entrega para el ID: {id_}. Continuando...")
        continue
    time.sleep(2)
    ...
    while True:
        if time.time() - start_time > MAX_DURATION:
            ...
            enviar_notificacion_slack(mensaje=f"El script ha finalizado por tiempo máximo MÉXICO.")
            driver.quit()
            sys.exit()
